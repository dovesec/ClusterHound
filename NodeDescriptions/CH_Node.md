...
A Kubernetes worker or control plane node. Enriched with OS, kernel, kubelet version and internal IP where the nodes API is accessible. Synthesised as a placeholder from `spec.nodeName` on Pod specs when the nodes API is inaccessible. Source of `CH_accessIMDS` edges to the cloud metadata endpoint where a provider is detected.
