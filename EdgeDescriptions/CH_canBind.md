...
Create or modify RoleBindings and ClusterRoleBindings. Kubernetes enforces that you can only bind a role if you already hold all the permissions it grants, or if you have been explicitly granted the `bind` verb on that specific role. Without either condition, the API server will reject the request, meaning `CH_canBind` alone cannot escalate beyond your current permission set and is primarily useful for persistence.

The escalation path opens when combined with `CH_canEscalate`. Creating a new wildcard role and then binding it does not work. The binding check rejects it because you don't already hold `*/*` and you have no `bind` verb on the newly created role. Instead: find a low-privilege role you can currently bind, bind yourself to it first (the check passes while the role is still low-priv), then use `CH_canEscalate` to escalate that same role to wildcard permissions. The existing binding now covers the escalated permissions.
