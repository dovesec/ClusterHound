...
If no existing binding is exploitable, `canBind` on the same identity unlocks an alternative: create a binding to any low-privilege role first (the binding check passes while the role is still within your permission set), then patch that role to wildcard.
